...
func openImpl(rt *goja.Runtime, mr *moduleResolver, fs afero.Fs, basePWD *url.URL, filename string, args ...string) (goja.Value, error) {
	...
	if len(args) > 0 && strings.Contains(args[0], "b") {
		dataModule, exists := mr.goModules["k6/data"]
		if !exists {
